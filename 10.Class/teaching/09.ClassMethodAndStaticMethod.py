...
class ExerciseSession:
    ...
    def __repr__(self):
        ...
        return s
    @classmethod # class method decorator (decorators change behaviour of following line function)
    def instanciate_from_csv(cls):
        with open("exercises.csv") as f1:
            reader = csv.DictReader(f1)
            exercises = list(reader)
            for exercise in excersices:
                ExerciseSession(name=excercise.get("name"), intensity=exercise.get("intensity")
                ,duration=int(exercise.get("duration")))
    @staticmethod
    def is_integer(num):
