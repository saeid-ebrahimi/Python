...
def insert_row_into_courses(name , category ,company_id, connection):
    ...
    if check_exits is None:    
        ...
        print("course with id {} and name {} already exists!!".format(*check_exits,name))
...
def insert_row_into_companies(name,connection):
    ...
    if check_exits is None:
        ...
        print("company with id {} and name {} already exists!!".format(*check_exits, name))
def delete_row_from_companies(name,connection):
    sql_command = "DELETE FROM companies WHERE name=?"
    cursor = connection.cursor()
    check_exits = cursor.execute("SELECT id FROM companies WHERE name=?", (name,)).fetchone()
...
def delete_row_from_courses(name, connection):
    sql_command = "DELETE FROM courses WHERE name=?"
    cursor = connection.cursor()
    check_exits = cursor.execute(
...
sql = '''SELECT companies.name, courses.name 
FROM companies 
LEFT JOIN courses ON 
companies.id = courses.company_id'''
...
FROM companies
INNER JOIN courses
ON companies.name = "Mammoth Interative"
"""
cursor.execute(sql)
print(cursor.fetchall())
